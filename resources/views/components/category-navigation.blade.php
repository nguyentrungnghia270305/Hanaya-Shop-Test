...
<div class="bg-white rounded-lg shadow-md p-4 mb-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <!-- Paper Flower -->
        <a href="{{ route('user.products.index', ['category_name' => 'paper-flower']) }}" 
           class="category-item group relative overflow-hidden rounded-lg border-2 transition-all duration-300 hover:shadow-md {{ $currentCategoryName === 'paper-flower' ? 'border-pink-500 bg-pink-50' : 'border-gray-200 hover:border-pink-300' }}">
            <div class="p-3 text-center">
                <div class="w-12 h-12 mx-auto mb-2 rounded-full flex items-center justify-center {{ $currentCategoryName === 'paper-flower' ? 'bg-pink-500' : 'bg-gray-100 group-hover:bg-pink-100' }}">
                    <svg class="w-6 h-6 {{ $currentCategoryName === 'paper-flower' ? 'text-white' : 'text-gray-600 group-hover:text-pink-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                </div>
                <h4 class="text-sm font-semibold {{ $currentCategoryName === 'paper-flower' ? 'text-pink-700' : 'text-gray-700 group-hover:text-pink-600' }}">
                    Paper Flowers
                </h4>
            </div>
        </a>
