        ...
        <div class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
            @if ($products->count() > 0)
                @foreach ($products as $productItem)
                    <div class="bg-white rounded-lg shadow-md hover:shadow-xl overflow-hidden transition-all duration-300 flex flex-col transform hover:scale-105">
                        <div class="relative">
                            <div class="aspect-[3/4] w-full bg-gray-100 overflow-hidden">
                                <img src="{{ $productItem->image_url ? asset('storage/' . $productItem->image_url) : asset('images/no-image.png') }}" 
                                     class="w-full h-full object-cover transition-transform duration-300 hover:scale-110" 
                                     alt="{{ $productItem->name }}"
                                     loading="lazy">
                            </div>
                            @if($productItem->discount_percent > 0)
                            <span class="absolute top-2 left-2 bg-red-600 text-white text-xs px-2 py-1 rounded-full font-semibold">
                                -{{ $productItem->discount_percent }}%
                            </span>
                            @endif
                            <button class="absolute top-2 right-2 bg-white bg-opacity-80 text-gray-600 hover:text-red-500 p-2 rounded-full transition-colors">
                                <i class="far fa-heart text-sm"></i>
                            </button>
                        </div>
                        <div class="p-3 sm:p-4 flex-1 flex flex-col justify-between">
                            <div>
                                <h3 class="text-sm sm:text-base font-semibold text-gray-800 line-clamp-2 mb-2">{{ $productItem->name }}</h3>
                                @if($productItem->category)
                                <p class="text-xs text-pink-600 font-medium mb-1">{{ $productItem->category->name }}</p>
                                @endif
