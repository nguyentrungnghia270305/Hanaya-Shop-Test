<x-app-layout>
    <div class="max-w-5xl mx-auto py-8">
        <h2 class="text-3xl font-bold text-gray-800 mb-8 text-center">{{ __('Posts about flowers & products') }}</h2>
        
        <!-- Search Form -->
        <div class="mb-8 flex justify-center">
            <form method="GET" action="{{ route('posts.index') }}" class="flex gap-2 w-full max-w-md">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search posts..." class="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500">
                <button type="submit" class="bg-pink-600 text-white px-4 py-2 rounded hover:bg-pink-700">Search</button>
                @if(request('search'))
                    <a href="{{ route('posts.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @forelse($posts as $post)
                <div class="bg-white rounded-lg shadow p-6 flex flex-col">
                    ...
                    <a href="{{ route('posts.show', $post->slug) }}" class="mt-4 text-pink-600 hover:underline">Read more</a>
                </div>
            @empty
